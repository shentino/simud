...
int main(string str) {
   ...
   if( legal_filename(a = this_player()->make_path_absolute(str)) && file_size(a) == -2 ) {
      ...
      foreach( b : get_dir(a) ) {
         if( ob = find_object(a+b) ) {
            if( ob == find_object("/world/world") )
               msg("~CWRNIf you want to update ~CBRT/world/world~CWRN, do it manually.~CDEF");
            else
               destruct(ob);
         }
      }
   ...
   // Give precedence to updating clones.
   ob = present( str, this_player() );
   if( !ob && environment(this_player()) ) ob = present( str, environment(this_player()) );
   if( !ob ) ob = find_room( str, this_player() );
   if( !ob ) ob = find_object( str );
   if( !ob ) ob = find_object( this_player()->make_path_absolute(str) );
