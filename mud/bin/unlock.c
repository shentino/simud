string query_action_name( mixed *action ) {
   return "Unlocking " + action[B_TARGET]->query_specific();
}
...
int main(string param)
{
   ...
   if( !param)
      return notify_fail("What do you want to unlock, and with what?\n");
...
void on_action( object ob, string b ) {
   ...
   if( !key->on_unlock(ob) )
      msg("You can't use "+key->query_specific()+" for unlocking that.\n");
}