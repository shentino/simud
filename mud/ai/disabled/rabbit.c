...
/// \return 1 if the kitten is hungry (below 50% food).
int is_hungry() { return (this_object()->query_food() < this_object()->query_food_capacity() / 2); }

/// Perform the Kitten's actual AI tick (overrides method in ../ai.c)
void on_tick() {
   ...
   if (cact == "hunt" && victim->query_dead()) {
      command("emote sniffs at the victim warily.");
...
void on_soul(int attribute, string param, string verb) {
   ...
   if (verb == "pet") {
      if (anger[(string)this_player()] > 0) {
         ...
         command("emote wrinkles its nose at "+this_player()->query_distant());
...
void act_emote() {
   ...
   if (!victim) {
      ...
      victim = 0;
      command("emote scratches at the ground with its hind leg.");
      ...
      switch (random(5)) {
         case 0: command("nuzzle "+victim->query_name()); break;
         case 1: command("stare "+victim->query_name()); break;
         case 2: command("rub "+victim->query_name()); break;
         default: command("emote scratches at the ground with its hind leg."); break;
      }
      boredom++;
   }
   end_act();
...
void act_hunt() {
   ...
   foreach(food: all_inventory(room))
      if (food->query_edible()) {
         ...
         if (this_object()->query_adjacent(food)) {
            ...
            command("emote stares hungrily at "+food->query_name());
         end_act();
         return;
      }
   ...
   if (machine = present("vending machine",room)) {
      ...
      if (this_object()->query_adjacent(machine)) {
         ...
         command("emote stares hungrily at "+machine->query_name());
      end_act();
   } else { // worst case, food must be hunted
      foreach (food: all_inventory(room)) {
         if (food->query_race() == "rodent")
            continue;
