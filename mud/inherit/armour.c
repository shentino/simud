...
varargs int query_requirements_met(object player, int silent) {
  ...
  foreach (name: m_indices(requirements)) {
    switch (name) {
      ...
      case "skill":
        if (player->query_skill(armour_type) < requirements[name]) {
...
int on_craft( object *materials ) {
   ...
   foreach( ob : materials ) {
      ob->apply_armor_properties( this_object() );
      this_object()->set_weight( this_object()->query_weight() + ob->query_weight() );
   }
   return 0;
}
