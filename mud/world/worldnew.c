...
varargs object full_restore_object( string file, int force_uid ) {
   ...
   do {
      ...
      if( "/world/world" == file_name )
         ...
         curr_ob = clone_object( file_name );
      if( load_name(curr_ob) == "/world/maparea" )
         log_file("world", "Maparea loaded: " + as_lpc(curr_ob) + " (new_depth " + new_depth + ", depth " + depth + "), env="+as_lpc(env)+"\n");
      // If it's the first object, then it's the root object, which we return.
      if( !base_ob )
         base_ob = curr_ob;
      // Set its unique id
      if( force_uid ) curr_ob->set_unique_id( uid );
//      log_file("world", "After set_unique_id, curr_ob is " + as_lpc(curr_ob) + ", id is " + curr_ob->query_unique_id() + "\n" );
      ...
      while( depth > new_depth ) {
         if( load_name(curr_ob) =="/world/maparea" || load_name(curr_ob) == "/world/world" )
            log_file("world", "ob="+as_lpc(curr_ob)+", depth="+depth+", new_depth="+new_depth+", env="+as_lpc(env)+"\n");
         depth--;
...
//      log_file("world", "load_name is " + load_name(curr_ob)+"\n");
      ...
      if( env ) {
         if( load_name(curr_ob) =="/world/maparea" )
            log_file("world", "ob="+as_lpc(curr_ob)+", env="+as_lpc(env)+"\n");
         curr_ob->move(env);
      }
...
void world_timed_save() {
   ...
   msg_all("[~CTITWorld~CDEF] ~CWRNSaving world state! Please wait, this may take a little while.~CDEF");
...
//   limited( (: ::save_self(); :), LIMIT_UNLIMITED, 0 );
   if( ob = find_room(";bank") )
      ob->presave();
   else {
      ob = clone_object("/obj/bank");
      ob->move(this_object());
      ob->presave();
   }
   limited( (: full_save_object("/world/world.obj", this_object()) :),
      LIMIT_UNLIMITED, 0 );
   if( ob )
      ob->postsave();
   call_out("report_delay",1);
}

/// Say how long the save actually took
void report_delay() {
   int time = time() - save_start_time - 1;
   if (time > 59) {
     int now, five, ten;
     catch(sscanf(read_file("/proc/loadavg"),"%s %s %s ", now, five, ten));
     msg_all("[~CTITWorld~CDEF] Time elapsed: ~CTIT"+time+"~CDEF. "+
...
// arg is ignored, only load from the correct file.
   limited((: ::load_self(1) :) , LIMIT_UNLIMITED, 0 );
}
...
void create() {
   ...
   call_out("world_timed_save", WORLD_SAVE_TIME);
   ::create();
   // Note: Moved down to here! Hope this doesn't cause problems.
//   load_self( 1 );
   limited( (: full_restore_object("/world/world.obj", 1) :),
      LIMIT_UNLIMITED, 0 );
   log_file("world", "World load finished OK\n");
   "/secure/odin"->check_id(1);
   load_in_progress = 0;
}
...
string destructor( object ob ) {
   if( !"/obj/shut"->query_shutdown_in_progress() )
      return "The world object may not be destructed except during shutdown.";
