...
int find_closest( mapping target_area, int target_coord, int unclipped ) {
   int closest, curr_node, closest_dist, curr_dist, player_coord;

   closest = 0;
   player_coord = this_player()->query_coord();

   foreach( curr_node : target_area ) {
      if( !unclipped && curr_node != player_coord + MAKE_C(4096,4096,32) &&
         !member(FLOOR_CHARS,environment(this_player())->get_tiletype(CX(curr_node)+CX(target_coord)-4096, CY(curr_node)+CY(target_coord)-4096))  ) {
//         write(MAKE_CSTR(curr_node) + " is " + environment(this_player())->get_tiletype(CX(curr_node)-4096, CY(curr_node)-4096) +
 ...
 */
varargs void init_walk_action( mapping target_area, int target_coord, mixed arrive_action, int unclipped, int prepush ) {
   int dest_coord;
   ...
   if( target_coord == -1 ) {
      this_player()->push_action(
         ({ 700, this_object(), this_player(), ST_STANDING, ({ target_area, -1, arrive_action, -1, unclipped }) }) );
      return;
   }

   // Early exit case for invalid shapes and invaild room maps.
   if( !target_area || !environment(this_player())->query_zbuf() ) {
      if( prepush )
         this_player()->set_action( -1, arrive_action );
      else
         this_player()->push_action( arrive_action );
      return;
   }

   // Early exit for "already there" -- saves some grief.
   foreach( dest_coord : target_area ) {
//      write("Local check: dest_coord="+MAKE_CSTR(dest_coord)+", tp()+targ="+MAKE_CSTR(this_player()->query_coord()+target_coord)+"\n");
      if( CX(dest_coord) + CX(target_coord) - 4096 == this_player()->query_x() &&
         CY(dest_coord) + CY(target_coord) - 4096 == this_player()->query_y() ) {
         if( prepush )
            this_player()->set_action( -1, arrive_action );
         else
            this_player()->push_action( arrive_action );
         this_player()->pop_action();
         return;
      }
   }

   // Make sure they're not entangled or grappling
   if( single_present("grapple_ob", this_player()) ) {
      msg("You can't walk while grappled!");
      return;
   } else if ( sizeof(this_player()->query_env_var("grappling")) ) {
      msg("You can't walk while grappling!");
      return;
   } else if ( single_present("entangle_ob", this_player()) ) {
      msg("You are being prevented from walking right now.");
      return;
   }

   // First off, find where we're walking to.
//   msg("Shape to seek to is: "+as_lpc(map(environment(this_player())->get_empty_shape(target_coord, target_area), (: MAKE_CSTR($1) :))));
//   msg("target_coord is " + MAKE_CSTR(target_coord) + ", target_shape is " + as_lpc(map(target_area, (: MAKE_CSTR($1) :))) );
   if( !unclipped )
      target_area = environment(this_player())->get_empty_shape( target_coord, target_area );
//   msg("target_area is " + as_lpc(map(target_area, (: MAKE_CSTR($1) :))) );
   dest_coord = find_closest( target_area, target_coord, unclipped );
   ...
   while( !haspath && sizeof(param[0]) ) {
      ...
      if( !haspath ) {
         ...
         if( this_player()->query_is_player() )
            debug(as_string(this_player()) + ": Finding path to " + CX(param[1]) + ", " + CY(param[1]) + " fails.", "map" );
         ...
         param[0] -= old_coord;
         param[1] = find_closest( param[0], param[3], param[4] );
         if( param[1] == -1 ) break;
      }
   }
