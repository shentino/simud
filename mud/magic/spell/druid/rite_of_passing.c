...
/**
 * This spell is not yet ready... don't touch it, eh? :)
 */

string query_name() { return "rite of passing"; }

int query_must_hold_target() { return 0; }
...
string query_step_skill(mixed param) {
	...
	int step = param["step"] % sizeof(skills);
	return( skills[step] );
}

string validate_params(mapping param) {
  if( param["target"]->query_is_living() )
    return "You can only perform the rite on corpses.";
  if( param["target"]->query_quantity() > 1 )
    return "You may only pass one spirit at a time.";
  if( environment(param["target"]) == param["caster"] )
    return "You must not be holding the corpse.";
  else
    return "ok";
}
