...
string query_name() { return "shock bolt"; }
...
varargs int on_cast(mapping param) {
  ...
  dc = target->skill_check("magic.technique.evoking", 25, 50, 75) * 10 + target->query_stat("spd");
  wc = ([ "electricity" : 3 + caster->skill_check("magic.mana.lightning",dc,dc+25,dc+50) ]);
