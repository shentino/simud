...
void end() {
  if (victim)
    msg_object(victim,"~CBRTYou feel the energy returning to your bloodstream.~CDEF");
...
//  move_object(this_object(),target); // This is NEVER legal.
   ...
   victim = target;
   msg_object(victim,"~CBRTYou feel a gnawing ache coursing through your veins and you feel a bit faint.~CDEF");
   set_heart_beat(1);
}
