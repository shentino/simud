...
void tell_user(string messg)
{
    if (attached)
	tell_object(user, messg);
    else
	buff=sprintf("%s%s", buff, messg);
    if (!attached && member(messg, '') != -1)
	tell_object(user, sprintf("[telnet: Beep from %s%c%s]\n",
	    host,
	    port == DEFAULT_PORT ? 0 :' ',
	    port == DEFAULT_PORT ? "":to_string(port)));
    if (log_file)
	write_file(log_file, messg);
    return;
}
...
{
    debug("callback("+fd+","+act+","+to_string(a)+","+to_string(b)+")");
    switch(act) {
      case SOCKET_READY:
...
//	      str=eval_path(trimstr(str[2..]));
	      str=this_player()->make_path_absolute(str[2..]);
	      if (file_size(str) == -2)
