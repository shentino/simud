...
void do_bleeder( object victim, int damage ) {
   object ob, *obs;
   obs = filter( all_inventory(victim), (: $1->query_is_bleeder() && $1->query_type() == "acid" :) );
   if( sizeof(obs) ) ob = obs[0];
   if( !ob ) {
      ob = clone_object("/battle/bleeder_object");
	  ob->set_dam_type("acid");
	  ob->set_hit_msg("Your flesh burns from the acid.");
	  ob->set_end_msg("The acid finally stops burning.");
   ...
   msg_object(victim,"~[100Acid from the bite burns your flesh!~CDEF");
}
...
void on_action( object victim ) {
   ...
   switch (result[RESULT_TYPE]) {
     case "miss":
       buf = "~[130Digestive fluids drip from ~npos teeth.";
       break;
     default:
       if (damage == 100)
