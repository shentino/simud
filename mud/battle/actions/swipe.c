...
void on_action( object victim ) {
   ...
   if( !BATTLE_DAEMON->check_legal_melee(victim, "swipe") )
      return;
   ...
   switch (result[RESULT_TYPE]) {
     case "miss":
       buf = "~[130~Name ~verbmiss ~targ with ~poss wild swing!";
       break;
/*
     case "block":
     case "parry":
...
*/
     default:
       if (damage >= 10)
         buf = "~[090~Name ~verbslash ~targ with ~poss "+weapon->query_name()+".";
       else if (damage > 0)
         buf = "~[090~Npos swordchucks slam into ~targ";
       else
