...
void on_action( object victim ) {
   ...
   if( !BATTLE_DAEMON->check_legal_melee(victim, "smash2") )
      return;
   ...
   wc["blunt"] += this_player()->query_str_wc() + 4 +
   ...
   switch (result[RESULT_TYPE]) {
     ...
     default:
       ...
       if (damage >= 40)
         buf = "~[090~Name ~verbdeliver a frightening ~[150SMASH~[090 to ~npot cranial region!";
       else if (damage >= 20)
         buf = "~[090~Name ~verbsmash ~targ with ~poss flail!";
       else if (damage > 0)
         buf = "~[090~Name ~verbslam ~targ with ~poss "+weapon->query_name()+".";
       else
