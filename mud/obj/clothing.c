...
int on_drop( object player ) {
   ...
   if( query_held() ) {
      set_held( 0 );
      player->msg_local( "~CACT~Name ~verbtake off " + query_specific() + " and ~verbdrop it.~CDEF" );
...
int on_give( object giver, object receiver ) {
   ...
   if( query_held() ) {
      set_held( 0 );
      giver->msg_local( "~CACT~Name ~verbtake off " + query_specific() + " and ~verbgive it to " + receiver->query_specific() + ".~CDEF" );
      return MOVE_SILENCE_FLAG | MOVE_OK_FLAG;
   }
   else return MOVE_OK_FLAG;
}
