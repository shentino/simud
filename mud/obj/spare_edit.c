...
void do_backspace() {
   ...
   if( rc == 0 ) {
      ...
      edited_text = edited_text[0..rl-1] + edited_text[rl+1..];
      refresh_scroll = 1;
      total_lines--;
   }
   else {
      ...
      if( rc > strlen(edited_text[rl]) ) cursorx--;
         ...
         if( rc == 1 ) edited_text[rl] = edited_text[rl][1..];
            ...
            edited_text[rl] = edited_text[rl][0..rc - 2] + edited_text[rl][rc..];
         cursorx--;
...
void do_enter() {
   ...
   if( rc == 0 ) {
      if( rl <= 0 ) edited_text = ({ "" }) + edited_text;
      else edited_text = edited_text[0..rl-1] + ({ "" }) + edited_text[rl..];
   }
   else if( rc >= strlen(edited_text[rl]) )
      edited_text = edited_text[0..rl] + ({ "" }) + edited_text[rl+1..];
   else {
      if( rl == 0 )
...
void keypress( string c ) {
   ...
   if( comm_code != KEY_NONE ) {
      ...
      if( comm_code == KEY_TAB ) {
         type_char(" ");
         while( cursorx % tab_size != 0 ) type_char(" ");
      }
