...
int alias(string str) {
   ...
   // confusion immunity
   if (member( ({ "alias", "realias", "unalias" }), query_verb() ) &&
      (akey == "alias" || akey == "unalias" || akey == "realias")) {
      write("==> [Alias] For sanity's sake, aliasing 'alias', 'unalias', and 'realias' is not allowed.\n");
      return 1;
   }

   // bang block
   if ((query_verb() == "alias" || query_verb() == "realias")
      && akey && aval && (akey[0..0] == "!" || aval[0..0] == "!")
   ) {
   ...
   if ( query_verb() == "realias" ) {
      if (!akey || !aval) {
         msg("==> [Alias] Usage:  realias <old alias> <new alias>.");
         return 1;
      }

      if (akey == aval) {
         msg("==> [Alias] You cannot rename an alias into itself.");
         return 1;
      }

      if (!member(aliases, akey)) {
         msg("==> [Alias] Entry for " + akey + " does not exist.");
         return 1;
      }

      if (member(aliases, aval)) {
         msg("==> [Alias] Entry for " + aval + " already exists.");
         return 1;
      }

      aliases[aval] = aliases[akey];
      efun::m_delete(aliases, akey);

      msg("==> [Alias] Entry for " + akey + " was renamed to " + aval + ".");
      return 1;
   }
